...
module Rollbar
  class Sidekiq
    ...
    def skip_report?
      ...
      notifiy_on_failure_number.nil? ? skip_globally? : skip_override?
    end
    ...
    def skip_override?
      case
      when notifiy_on_failure_number.is_a?(Integer)
        notifiy_on_failure_number != retry_count
      when notifiy_on_failure_number.is_a?(Array)
        !notifiy_on_failure_number.include?(retry_count)
      end
    end

    def notifiy_on_failure_number
      @notifiy_on_failure_number ||= worker_instance.notifiy_on_failure_number rescue nil
    end
